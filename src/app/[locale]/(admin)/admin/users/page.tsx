...
export default async function AdminUsersPage({
  ...
  const table: Table = {
    columns: [
      ...
      {
        name: 'roleplayFreePlay',
        title: t('fields.roleplay_free_play'),
        callback: async (item: User) => {
          const enabled = await hasActiveRoleplayFreePlay(item.id);

          return enabled ? (
            <Badge variant="default">{t('fields.enabled')}</Badge>
          ) : (
